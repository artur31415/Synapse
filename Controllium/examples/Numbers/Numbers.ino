...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
[ENG-USA]
    In this example the number buttons, that are on the Controllium android app, are used on the ESP8266 to set its pins
    states to the buttons states.

[PT-BR]
    Neste exemplo os botões de numeros, que estão no aplicativo android Controllium, são usados na ESP8266 para definir
...
//Controllium object
Controllium controllium("ESP_Numbers");//Name of this device, if you have more than one connected, provide a different name to each
...
void loop()
{
  ...
  if(controllium.Update(200))//Returns true if a new msg was received
  {
    ...
    if(number0Value >= 0)
      digitalWrite(D[4], (int)number0Value);

    if(number1Value >= 0)
      digitalWrite(D[5], (int)number1Value);

    if(number2Value >= 0)
      digitalWrite(D[6], (int)number2Value);

    if(number3Value >= 0)
      digitalWrite(D[7], (int)number3Value);
  }
}
